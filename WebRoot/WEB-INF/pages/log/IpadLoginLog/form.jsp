...
<%@ include file="/page/head.html" %>
	<div class="body-box">
		<div class="rhead">
			<div class="rpos">当前位置: iPad登录日志 - ${id eq null?"新增":"修改" }</div>
			<div class="ropt"><input type="button" value="返回列表" class="ps-btn" onclick="history.back();" /></div>
			<div class="clear"></div>
		</div>
		<form method="post" action="${ctx }/log/ipad-login-log!save.action" id="form">
			...
			<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
				...
				<tr>
					...
					<td  width="38%" class="pn-fcontent">
						...
						<c:if test="${id eq null}"><input type="text" name="id" value="${id }" validate="{required:true,number:true,remote:'${ctx }/log/ipad-login-log!checkUnique.action?propertyName=id',messages:{remote:'此ID已被使用'}}"/></c:if>							
					</td>
				...
				<tr>
					...
					<td colspan='3' width="38%" class="pn-fcontent">
						<input type="text" name="status" value="${status }" validate="{maxlength:200}"/>
					</td>
				</tr>
				<tr>
