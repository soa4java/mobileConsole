...
<%@ include file="/page/head.html" %>
	<div class="body-box">
		<div class="rhead">
			<div class="rpos">当前位置: 资源权限 - ${entity.id eq null?"新增":"修改" }</div>
			<div class="clear"></div>
		</div>
		<form method="post" action="${ctx }/authority/save" id="form">
			...
			<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
				...
				<tr>
					...
					<td  width="38%" class="pn-fcontent">
						<c:if test="${entity.id ne null}"><input type="hidden" name="code" value="${entity.code }"/>${entity.code }</c:if>
						<c:if test="${entity.id eq null}"><input type="text" name="code" value="${entity.code }" validate="{required:true,maxlength:20,remote:'${ctx }/acl/authority!checkUnique.action?propertyName=code',messages:{remote:'此资源代码已被使用'}}"/></c:if>							
					</td>
			...
			<tr>
				...
				<td  colspan='3' width="38%" class="pn-fcontent">
					...
					<c:forEach items="${list }" var="opt">
							<tr class="authority">
								<td style="width: 45%;"><input type="text" name="optCode" value="${opt.code }" class="text"/></td>	
								<td style="width: 45%;"><input type="text" name="optName" value="${opt.name }" class="text"/></td>
								<td>
									<input type="hidden" name="optId" value="${opt.id }"/>
						...
						<c:if test="${list eq null }">
							<tr class="authority">
								<td><input type="text" name=optCode class="text" validate="{maxlength:20}"/></td>	
				...
				<tr>
					<td colspan="4" class="pn-fbutton">
						<input type="button" value="提交" class="ps-btn" onclick="submit();"/>&nbsp; 
